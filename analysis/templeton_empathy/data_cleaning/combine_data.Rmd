...
colombia_empathy_data <- as_tibble(read.csv("Colombia_Master_MAY.csv"))
colombia_altruism_data <- as_tibble(read.csv("dictator_colombia_merge.csv"))
```

Clean task data
```{r}
colombia_altruism_data <- colombia_altruism_data %>% mutate(unique_id = paste0("columbia_", Subject)) %>%
  select(unique_id, share.RESP) %>%
  rename (dictator_shared = share.RESP)
colombia_empathy_data <- colombia_empathy_data %>% select(ï..Subject, Empathy_Pain_RESP, Empathy_Sorry_RESP) %>%
  mutate(unique_id = paste0("columbia_", ï..Subject)) %>%
  select (-ï..Subject)
clean_colombia_task_data <- full_join(colombia_altruism_data, colombia_empathy_data, by = "unique_id") %>%
  mutate(country = "colombia")
clean_task_data <- task_data %>% select(Subject, ï..Country, Empathy_Pain_RESP, Empathy_Sorry_RESP, Dictator_Shared)
...
colnames(clean_colombia_task_data) <- tolower(colnames(clean_colombia_task_data))
clean_task_data <- distinct(clean_task_data)
clean_task_data <- rbind(clean_task_data, distinct(clean_colombia_task_data))
